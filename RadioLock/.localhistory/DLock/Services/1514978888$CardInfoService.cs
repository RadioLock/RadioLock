...
namespace LLock
{
    public class CardInfoService : ServiceBase<CardInfo2>
    {
        ...
        public string getLockNo(string roomName)
        {
            ...
            LLockConnector.ConnectionString = String.Format("Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0}" + fileDb + ";Jet OLEDB:Database Password=" + password + ";", Settings.Default.LockFolder);
            WriteLog("ConnectionString:" + LLockConnector.ConnectionString);
            string _LockNo = "";
            ...
            using (OleDbConnection connection = new OleDbConnection(LLockConnector.ConnectionString))
        ...
        protected override object Run(CardInfo2 request)
        {
            string password = ConfigurationSettings.AppSettings["dbPassword"];
            string fileDb = ConfigurationSettings.AppSettings["fileDb"];
            ELockConnector.ConnectionString = String.Format("Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0}" + fileDb + ";Jet OLEDB:Database Password=" + password + ";", Settings.Default.LockFolder);
            WriteLog("ConnectionString:" + ELockConnector.ConnectionString);
            var response = new CardInfoResponse2 { Result = 1 }; //0: OK
            if (!frmMain.isValid) return response;
            ELockConnector obj = new ELockConnector();

            switch (this.Request.PathInfo)
            {
                case "/readcard": //read card info
                    var Info = obj.ReadCard1();
